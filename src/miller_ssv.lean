...
def simple_stable_voting_new {χ υ : Type*} (prof : election_profile χ υ)
  [decidable_eq χ] [∀ v, decidable_rel (prof.Q v)] : finset χ :=
simple_stable_voting'' prof.voters prof.Q prof.candidates.card prof.candidates rfl prof.cpos
...
lemma ssv_singleton_new {χ υ : Type*} (prof : election_profile χ υ) (hcands : prof.candidates.card = 1)
  [decidable_eq χ] [∀ v, decidable_rel (prof.Q v)] : 
  simple_stable_voting_new prof = prof.candidates :=
by simp only [simple_stable_voting_new, simple_stable_voting'', hcands]
...
begin
  ...
  simp only [simple_stable_voting, simple_stable_voting', hn, true_and, finset.mem_attach, 
    exists_prop, exists_and_distrib_right, prod.mk.inj_iff, 
    exists_eq_right_right, exists_eq_right, finset.mem_image, subtype.exists, finset.mem_filter,
    finset.filter_congr_decidable, subtype.coe_mk, 
    subtype.val_eq_coe, prod.exists, finset.mem_product] at a_in,
  rcases a_in with ⟨b,⟨⟨p_in,hp⟩,ha⟩⟩,
  exact (finset.mem_product.1 p_in).1,
end

lemma mem_cands_of_mem_winners_new  {χ υ : Type*} (prof : election_profile χ υ) (a : χ)
  [decidable_eq χ] [∀ v, decidable_rel (prof.Q v)] : 
  a ∈ simple_stable_voting_new prof → a ∈ prof.candidates := 
begin
  intro a_in,
  by_cases hcands : prof.candidates.card = 1, 
  { rwa ssv_singleton_new prof hcands at a_in, },
  obtain ⟨n, hn⟩ := exists_eq_plus_two prof.candidates.card (by linarith [prof.cpos]) hcands, 
  simp only [simple_stable_voting_new, simple_stable_voting'', hn, 
...
begin
  ...
  set viable_set : finset (χ × χ) := (prof.candidates.product prof.candidates).filter
    (λ p, p.1 ∈ simple_stable_voting' prof.voters prof.Q d.succ (prof.candidates.erase p.2) sorry (nat.succ_pos d))
    with hvs,
  ...
  { use ⟨a,b⟩,
    ...
    simp only [simple_stable_voting, h_prof', h_erase_b, m_succ] at a_in,
    refine ⟨⟨(finset.mem_erase.1 this).2, b_in⟩, a_in⟩, },  
  obtain ⟨p, p_in, hp⟩ := exists_best_margin prof.voters prof.Q viable_nonempty,
  refine ⟨p.1, p.2, ⟨_,_⟩⟩,
  { rw hvs at p_in,
    simp only [finset.mem_filter] at p_in,
    split, {exact p_in.2, },
    simp only [p_in.left, prod.mk.eta], },
  { convert hp,
    rw hvs,
    sorry, },
end

lemma exists_ssv_winner'_new {χ υ : Type*} :
  ∀ (n : ℕ) (prof : election_profile χ υ), prof.candidates.card = n → 
  ∃ a, a ∈ simple_stable_voting_new prof :=
begin
  intro n,
  cases n with d, 
  { intros prof hn,
  ...
  simp only [simple_stable_voting_new, simple_stable_voting'', card_eq_d, 
    exists_prop, exists_and_distrib_right, exists_eq_right,
    finset.mem_image, finset.mem_filter, finset.filter_congr_decidable,
  ...
  { use ⟨a,b⟩,
    have : a ∈ (prof.candidates.erase b) := 
      by simp only [mem_cands_of_mem_winners_new prof' a a_in],
    simp only [simple_stable_voting_new, h_prof', h_erase_card b b_in, m_succ] at a_in,
